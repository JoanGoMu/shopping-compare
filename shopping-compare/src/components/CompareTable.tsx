...
export default function CompareTable({ products: initialProducts, allProducts = [], groupId, existingShareSlug, groups = [], onProductsChange }: Props) {
  ...
  const keysWithCoverage: SpecEntry[] = allSpecKeys.map((key) => ({
    key,
    count: filtered.filter((p) => ((p.specs as Record<string, unknown>) ?? {})[key] != null).length,
  }));
                        ...
                        <div className="aspect-[3/4] w-full bg-cream overflow-hidden relative">
                          ...
                          {p.price_check_failed && (
                            <div className="absolute inset-0 bg-white/60 flex flex-col items-center justify-center gap-1.5">
                              ...
                              <span className="text-[10px] text-ink font-medium text-center px-3 leading-tight">No longer available</span>
                            </div>
                          )}
                        </div>
                        {images.length > 1 && (
